  ...
  <div class="flex">
    ...
    >
      <div class="p-4 border-b flex items-center justify-between">
        <h1 class="text-xl font-bold ">Katha POS</h1>
      </div>
      <nav class="p-4">      
       
        <NuxtLink v-for="page in filteredPages" :to="page.link"  class="flex items-center py-2 px-4 rounded hover:bg-gray-100 "  active-class="bg-gray-200" >
          <span v-html="page.icon"></span>
          <span class="ml-2 md:inline-block">{{ page.name }}</span>
        </NuxtLink>
      </nav>
...
const listPages  = [
  {
    name : 'Dashboard',
    link : '/',
    icon : `<svg class="w-6 h-6 text-gray-800 dark:text-black" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    name : 'Kasir',
    link : '/pos',
    icon : `  <svg class="w-6 h-6 text-gray-800 dark:text-black" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
...
onMounted(async () => {
  ...
  role.value = await useUserRole();
   filteredPages.value = listPages.filter(page => !page.role || page.role === role.value)
  router.afterEach(() => {
    isOpen.value = false
  })
})
